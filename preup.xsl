...
<xsl:import href="../../openscap/xsl/security-guide.xsl" />
<xsl:import href="../../openscap/xsl/oval-report.xsl" />
<xsl:import href="../../openscap/xsl/sce-report.xsl" />
...
<xsl:variable name='toshow'>
  <xsl:choose>
    ...
    <xsl:otherwise>,pass,fixed,notchecked,informational,unknown,error,fail,needs_action,needs_inspection<xsl:value-of select='$show'/>,</xsl:otherwise>
  </xsl:choose>
</xsl:variable>
          ...
          <tgroup>
            <thead>
              <row>
                ...
                <entry>fixed</entry>
                <entry>fail</entry>
                <entry>needs inspection</entry>
                <entry>needs action</entry>
                <entry>error</entry>
            ...
            <tbody>
              <row role="result-legend">
                ...
                <entry align="center" role="result-needsinspection"><emphasis role="strong"><xsl:value-of select='count(cdf:rule-result[cdf:result="needs_inspection"])'/></emphasis></entry>
                <entry align="center" role="result-needsaction"><emphasis role="strong"><xsl:value-of select='count(cdf:rule-result[cdf:result="needs_action"])'/></emphasis></entry>
...
<xsl:template mode='rr.table' match='cdf:TestResult'>
  ...
  <table>
    ...
    <tgroup>
      ...
      <tbody>
          <xsl:apply-templates select='$results' mode='rr.table'>
              <xsl:sort select='normalize-space(cdf:result)'/>
          </xsl:apply-templates>
      </tbody>
    </tgroup>
  </table>
