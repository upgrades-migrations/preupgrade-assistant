...
class KickstartGenerator(object):
    ...
    def get_kickstart_repo(filename):
        ...
        for line in lines:
            fields = line.split('=')
            repo_dict[fields[0]] = fields[1] + '=' + fields[2]
        return repo_dict
    ...
    def update_repos(self, repos):
        self.ks_list = get_file_content(self.kick_start_name, 'r', method=True)
        idx = [i for i, word in enumerate(self.ks_list) if word.startswith('url')]
        cnt = int(idx[0]) + 1
        cnt = self.update_kickstart('# BEGIN This is pregenerated by Preupgrade Assistant. Your Review is needed.\n', cnt)
        for key, value in repos.iteritems():
            cnt = self.update_kickstart('repo --name={0} --{1}'.format(key, value), cnt)
        cnt = self.update_kickstart('\n# END\n', cnt)
        write_to_file(self.kick_start_name, 'w', self.ks_list)
    ...
    def generate(self):
        ...
        self.save_kickstart()
        self.update_repos(available_repos)
        #self.embed_script()
